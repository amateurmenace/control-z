# The Studio's infrastructure — a runbook, not a report
...
| Cloud Run `studio-api` (min-instances 0) | $0–5 | only when someone reads |
| Cloud Run Jobs `studio-pipeline` | ~$0 | only when the nightly job runs |
| GCS + CDN (editions, documents) | $1–5 | with the first pressing |
| Gemini embeddings + Flash passes (~50 meetings/mo) | $1–5 | only if `STUDIO_GEMINI_KEY` is set |
...
```bash
gcloud projects create community-ai-studio --name="Community AI Studio"
gcloud config set project community-ai-studio

# Attach billing. `gcloud billing accounts list` shows the open ones.
gcloud billing projects link community-ai-studio \
...
Console: <https://console.cloud.google.com/home/dashboard?project=community-ai-studio>
...
```bash
gcloud sql instances create studio-pg \
...
gcloud sql databases create studio --instance=studio-pg
gcloud sql users create studio --instance=studio-pg --password=<PICK ONE>
...
Console: <https://console.cloud.google.com/sql/instances?project=community-ai-studio>
...
> studio-pg --tier=db-g1-small`, a restart, and roughly double the line. Do it
...
```bash
printf '%s' '<gemini key>'   | gcloud secrets create studio-gemini-key --data-file=-
printf '%s' '<oauth client>' | gcloud secrets create studio-google-client-id --data-file=-
printf '%s' 'you@example.org' | gcloud secrets create studio-steward-allowlist --data-file=-
printf '%s' "$(openssl rand -hex 32)" | gcloud secrets create studio-service-token --data-file=-
printf '%s' 'postgresql://studio:<pw>@/studio?host=/cloudsql/community-ai-studio:us-east1:studio-pg' \
    | gcloud secrets create studio-dsn --data-file=-
```

Console: <https://console.cloud.google.com/security/secret-manager?project=community-ai-studio>
**Adds: ~$0.**

Each of these is optional and the Studio says so when one is missing: with no
...
```bash
gcloud artifacts repositories create studio \
    --repository-format=docker --location=us-east1

gcloud builds submit --tag us-east1-docker.pkg.dev/community-ai-studio/studio/api:1 \
    --file studio/Dockerfile .
```

Console: <https://console.cloud.google.com/artifacts?project=community-ai-studio>
...
gcloud run deploy studio-api \
    --image=us-east1-docker.pkg.dev/community-ai-studio/studio/api:1 \
    --region=us-east1 \
    --allow-unauthenticated \
    --min-instances=0 --max-instances=4 \
    --cpu=1 --memory=512Mi --concurrency=40 \
    --add-cloudsql-instances=community-ai-studio:us-east1:studio-pg \
    --set-secrets=STUDIO_DSN=studio-dsn:latest,\
STUDIO_GEMINI_KEY=studio-gemini-key:latest,\
STUDIO_GOOGLE_CLIENT_ID=studio-google-client-id:latest,\
STUDIO_STEWARD_ALLOWLIST=studio-steward-allowlist:latest,\
STUDIO_SERVICE_TOKEN=studio-service-token:latest
```

Console: <https://console.cloud.google.com/run?project=community-ai-studio>
...
```bash
gcloud run jobs create studio-migrate \
    --image=us-east1-docker.pkg.dev/community-ai-studio/studio/api:1 \
    --region=us-east1 \
    --add-cloudsql-instances=community-ai-studio:us-east1:studio-pg \
    --set-secrets=STUDIO_DSN=studio-dsn:latest \
    --command=python --args=-m,studio.migrate
gcloud run jobs execute studio-migrate --region=us-east1 --wait
```
...
```bash
gcloud storage buckets create gs://communityai-studio-edition \
    --location=us-east1 --uniform-bucket-level-access
gcloud storage buckets add-iam-policy-binding gs://communityai-studio-edition \
    --member=allUsers --role=roles/storage.objectViewer
```

Console: <https://console.cloud.google.com/storage/browser?project=community-ai-studio>
**Adds: $1–5/mo** including egress at launch scale.

`studio/press.py` uploads pre-gzipped objects with `Content-Encoding: gzip`,
...
- `communityai.studio/app/*` → the GCS bucket
- `communityai.studio/api/*` → Cloud Run `studio-api`
...
```bash
gcloud run jobs create studio-pipeline \
    --image=us-east1-docker.pkg.dev/community-ai-studio/studio/api:1 \
    --region=us-east1 \
    --add-cloudsql-instances=community-ai-studio:us-east1:studio-pg \
    --set-secrets=STUDIO_DSN=studio-dsn:latest,STUDIO_GEMINI_KEY=studio-gemini-key:latest \
    --command=python --args=-m,studio.connectors.youtube,--all-towns

gcloud scheduler jobs create http studio-nightly \
    --location=us-east1 --schedule="0 3 * * *" --time-zone="America/New_York" \
    --uri="https://us-east1-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/community-ai-studio/jobs/studio-pipeline:run" \
    --http-method=POST --oauth-service-account-email=<RUN_SA>
```

Console: <https://console.cloud.google.com/cloudscheduler?project=community-ai-studio>
**Adds: ~$0** (3 free jobs).
...
```bash
gcloud run jobs create studio-import \
    --image=... --region=us-east1 \
    --add-cloudsql-instances=... --set-secrets=STUDIO_DSN=studio-dsn:latest \
    --command=python --args=-m,studio.import_desk,--corpus,/corpus/corpus.db
```
...
```bash
cloud-sql-proxy community-ai-studio:us-east1:studio-pg --port 55432 &
STUDIO_DSN="postgresql://studio:<pw>@localhost:55432/studio" \
    .venv/bin/python -m studio.import_desk \
...
gcloud billing budgets create --billing-account=<ACCOUNT_ID> \
    --display-name="Community AI Studio" \
    --budget-amount=60USD \
...
The Studio also keeps its own ledger. `spend` records every AI call — model,
...
```bash
gcloud sql instances patch studio-pg --activation-policy=NEVER   # stops the $10–30
gcloud run services update studio-api --region=us-east1 --min-instances=0
```
...
To delete the lot: `gcloud projects delete community-ai-studio`. The record
