...
### P1 — the loop (fast follows) — ✅ wave 2 shipped 1.8.0 (§7, §8, §10)

7. **Publish from the desk.** ✅ **SHIPPED 1.8.0.** A "Publish the record" action on the desk's Memory page: runs the bake as a JobManager job into `site/docs/app`, shows the edition diff (meetings added, issues moved), and hands the human the push ritual. *Built:* `POST /api/memory/publish` reads the previous `manifest.json`/`search/meta.json` *before* the bake wipes the dir, presses as a `memory-publish` job, and returns `{meetings_added, count_deltas, corpus_hash, prev_hash, budget_busts}` + the push ritual; the button (`suite/static/js/memory.js`) renders the diff and a copyable deploy block. The web app footer already shows the edition date from `manifest.json`.
8. **Follows that survive.** ✅ **SHIPPED 1.8.0.** ☆ writes localStorage; the still-watching panel renders followed issues' resurfacings from the edition and offers each issue's RSS. Export/import follows as a small JSON (anti-lock-in even for preferences). *Built:* `/app/watching` + `page_still` + `stillWatching()`/`wireFollowIO()` in `app.js`; follows are `cz-follows` slugs mapped to `issues/{slug}.json`.
9. **The caption strip, translated.** Under the player, a synced caption line fed by the edition's VTT cues, following embed time via the iframe API's time messages; the language menu (seven + Simple English, when Interpreter tracks exist) drives both strip and transcript. AD text renders as an inline described-transcript view. *Acceptance:* strip drift ≤ 0.5 s across a 2-minute continuous play on the reference meeting. *(Wave-1 shipped the strip; the translated tracks arrive when Interpreter presses them into an edition.)*
10. **Offline, quietly.** ✅ **SHIPPED 1.8.0.** PWA manifest + service worker: shell precached, last-read meetings kept, an unassuming "read offline" affordance — the library-kiosk and thin-broadband case. Update banner in the house voice: *the record refreshed — reload for the new pressing.* *Built:* `manifest.webmanifest` + `sw.js` (cache keyed on the corpus fingerprint, deterministic, no wall-clock) emitted by `emit.py` `_write_pwa`; `registerSW()`/`updateBanner()` in `app.js`; CSP-clean (SW registration in app.js, not an inline script). **Also shipped beyond §P1:** documents + the roll-call ledgers + a **The votes** accountability page in the edition (specs/14 §11–12).
